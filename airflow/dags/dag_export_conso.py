...
# URL source et nom du fichier dans le bucket (hardcoded ici)
import_url = "https://data.enedis.fr/api/explore/v2.1/catalog/datasets/consommation-electrique-par-secteur-dactivite-departement/exports/csv?lang=fr&timezone=Europe%2FBerlin&use_labels=true&delimiter=%3B"
csv_gcs_file = "consommation-electrique-par-secteur-dactivite-departement.csv"

cloud_storage_path = f"{bucket_path}/{csv_gcs_file}"
local_file_path = f"/tmp/data/{csv_gcs_file}"
...
with DAG(
    ...
    def import_csv():
        ...
        try:
            subprocess.run(
                ['curl', '-s', '-o', local_file_path, import_url],
                check=True
            )
